...
class ModelTest(parameterized.TestCase):
  ...
  def test_decode_special_tokens(self):
    raw_tokenizer = tokenizers.P10Tokenizer()
    special_tokenizer = tokenizers.AddSpecialTokens(raw_tokenizer)
    decoder_vocab = vocabs.DecoderVocab(special_tokenizer)
  ...
  def test_multiobjective(self):
    ...
    self.assertEqual(model.decode_len, 12)  # 2 objectives * 6 tokens per obj
